...
import torch
from slang_gaussian_tomography.internal.tomography_tiled_slang import render_alpha_blend_tomo_tiles_slang_raw
def common_properties_from_inria_GaussianModel(gaussian_model):
  ...
  return xyz_ws, rotations, scales, density
...
def render(viewpoint_camera, pc, pipe, bg_color=torch.tensor([0., 0., 0.], device="cuda"), scaling_modifier = 1.0, override_color = None):
  """ Implements the volumetric tomography Interface defined in the inria code-base."""
  assert scaling_modifier == 1.0, "scaling_modifier is not supported in the slang-gaussian-tomography."
  assert override_color is None, "override_color is not support in the slang-gaussian-tomography."
  ...
  xyz_ws, rotations, scales, density = common_properties_from_inria_GaussianModel(pc)
  world_view_transform, proj_mat, cam_pos, fovy, fovx, height, width = common_properties_from_inria_Camera(viewpoint_camera)  

  render_pkg = render_alpha_blend_tomo_tiles_slang_raw(xyz_ws, rotations, scales,
