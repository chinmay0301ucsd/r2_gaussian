...
def training_report_wandb(
    ...
    if iteration in testing_iterations:
            ...
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    if idx in show_idx:
                        ...
                        image_show_2d.append(comparison)
                ...
                image_grid = np.concatenate(image_show_2d, axis=0)
                metrics_2d = {
                    f"{config['name']}/slice-gt_pred_diff": wandb.Image(image_grid),
                    f"{config['name']}/psnr_2d": psnr_2d,
                    f"{config['name']}/ssim_2d": ssim_2d
                }
                wandb.log(metrics_2d, step=iteration)
        ...
        for i in np.linspace(0, vol_gt.shape[2], 7).astype(int)[1:-1]:
            ...
            slices_3d.append(slice_viz)
        
        image_show_3d = np.concatenate(slices_3d, axis=0)
        
        metrics_3d = {
            "reconstruction/slice-gt_pred_diff": wandb.Image(image_show_3d),
            "reconstruction/psnr_3d": psnr_3d,
            "reconstruction/ssim_3d": ssim_3d,
            "scene/density_histogram": wandb.Histogram(scene.gaussians.get_density.cpu())
        }
